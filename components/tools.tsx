...
export function Tools() {
  ...
  return (
    <section className="py-12 border-t border-border">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-3 text-muted font-sans text-sm leading-relaxed">
        {tools.map((tool, index) => {
          ...
          return (
            <span key={tool.name} className="flex items-center">
              ...
              {index < tools.length - 1 && <span className="ml-4 text-border select-none">•</span>}
            </span>
          );
        })}
      </div>
